...
main_alter_record = ''
class MaterialInventory(QWidget):
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 基地物资盘点表 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
