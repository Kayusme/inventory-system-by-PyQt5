...
class OutProduct(QWidget):
    def __init__(self, parent=None):
        super(OutProduct, self).__init__()
        self.setUpUI()
    def setUpUI(self):
        # 查询数据
        ...
        self.addButton.setIcon(QIcon('.//image//产品出库.png'))
        ...
        searchCondision = ['按出库单号查询','按产品编号查询','按产品名称查询']
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 基地产品出库表 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
...
class addOutProduct(QDialog):

    def __init__(self, parent=None):
        super(addOutProduct, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.label_6 = QLabel("经手人:")
        self.label_7 = QLabel("出库时间:")
        self.label_8 = QLabel("片名:")
        self.label_9 = QLabel("收货方:")
        self.label_10 = QLabel("机构编号:")
    ...
    def handleTextChanged(self):
        ...
        cur = self.db.cursor()
        sql = "SELECT * FROM 基地产品出库表 WHERE 产品编号='%s'" % (text_1)
        cur.execute(sql)
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""or text_7 == ""or text_8 == ""or text_9 == ""or text_10 == ""):
            ...
            cur_0 = db_0.cursor()
            sql = "SELECT * FROM 公司机构设置表 WHERE 机构编号='%s'" % (text_10)
            cur_0.execute(sql)
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO  基地产品出库表 VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (str(id+1),text_1, text_2, text_3,text_4,text_5,text_6,text_7,text_8,text_9,text_10)
            cur.execute(sql)
            db.commit()
            #库存
            ...
            sql = "INSERT INTO 基地产品库存信息 VALUES ('%s','%s','%s','%s','%s','%s')" % (text_1, text_2, text_4,'0','0','无' )
            cur_c.execute(sql)
            db_c.commit()
            print(QMessageBox.information(self, "提示", "添加成功!", QMessageBox.Yes, QMessageBox.Yes))
            self.close()
