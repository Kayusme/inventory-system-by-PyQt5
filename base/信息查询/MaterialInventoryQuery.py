...
main_alter_record = ''
class MaterialInventoryQuery(QWidget):
    def __init__(self, parent=None):
        super(MaterialInventoryQuery, self).__init__()
        self.setUpUI()
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 物资库存信息表 WHERE %s LIKE '%s' ORDER BY %s " % (
...
if __name__ == "__main__":
    ...
    mainMindow = MaterialInventoryQuery()
    mainMindow.show()
    sys.exit(app.exec_())