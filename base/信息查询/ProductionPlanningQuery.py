...
main_alter_record = ''
class ProductionPlanningQuery(QWidget):
    def __init__(self, parent=None):
        super(ProductionPlanningQuery, self).__init__()
        self.setUpUI()
    ...
    def searchButtonClicked(self):
        ...
        if (conditionChoice == "按序号查询"):
            conditionChoice = '序号'
        elif (conditionChoice == "按农户姓名查询"):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 生产计划信息表 WHERE %s LIKE '%s' ORDER BY %s " % (
...
if __name__ == "__main__":
    ...
    mainMindow = ProductionPlanningQuery()
    mainMindow.show()
    sys.exit(app.exec_())