...
main_alter_record = ''
class ProductInventoryQuery(QWidget):
    def __init__(self, parent=None):
        super(ProductInventoryQuery, self).__init__()
        self.setUpUI()
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 基地产品库存信息 WHERE %s LIKE '%s' ORDER BY %s " % (
...
if __name__ == "__main__":
    ...
    mainMindow = ProductInventoryQuery()
    mainMindow.show()
    sys.exit(app.exec_())