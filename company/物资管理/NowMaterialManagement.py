...
main_alter_record = ''
class NowMaterialManagement(QWidget):
    def __init__(self, parent=None):
        super(NowMaterialManagement, self).__init__()
        self.setUpUI()
    def setUpUI(self):
        # 查询数据
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 物资库存信息表 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
    ...
    def addButtonClicked(self):
        ...
        addDialog = addNowMaterialManagement(self)
        addDialog.show()
        addDialog.exec_()
        self.begin()
        return
...
class addNowMaterialManagement(QDialog):

    def __init__(self, parent=None):
        super(addNowMaterialManagement, self).__init__(parent)
        global main_alter_record
    ...
    def buttonCicked(self):
        ...
        db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
        cur = db.cursor()
        sql = "UPDATE 物资库存信息表 SET 备注='%s' WHERE 物资编号='%s'" % (text_1, self.id)
        cur.execute(sql)
        db.commit()
...
if __name__ == "__main__":
    ...
    mainMindow = NowMaterialManagement()
    mainMindow.show()
    sys.exit(app.exec_())