...
class CustomerPurchaseStatistics(QWidget):
    def __init__(self, parent=None):
        super(CustomerPurchaseStatistics, self).__init__()
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        searchCondision = ['按顾客编号查询', '按购买物品查询', '按时间查询']
    ...
    def searchButtonClicked(self):
        ...
        if (conditionChoice == "按顾客编号查询"):
            conditionChoice = '顾客编号'
        elif (conditionChoice == "按购买物品查询"):
            conditionChoice = '购买物品'
        else:
            conditionChoice = '时间'
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 顾客购买 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
    ...
    def deleteButtonClicked(self):
        ...
        if graph == "柱状图":
            if way=="按不同品种统计":
                ...
                self.graphResults = pd.DataFrame(self.graphResults, columns=["a", "b", "c", "d", "e", "f", "g",'h'])
                self.graphResults.e = self.graphResults.e.apply(pd.to_numeric)
                self.x = list(self.graphResults.c.value_counts().keys())
                for x in self.x:
                    a = self.graphResults.e[self.graphResults.c == x].sum()
                    self.y.append(a)
...
if __name__ == "__main__":
    ...
    mainMindow = CustomerPurchaseStatistics()
    mainMindow.show()
    sys.exit(app.exec_())