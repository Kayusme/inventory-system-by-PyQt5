...
class manager(QWidget):
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 用户管理 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
    ...
    def deleteButtonClicked(self):
        ...
        if (QMessageBox.warning(self, "警告", "是否删除此行", QMessageBox.Yes, QMessageBox.No) ==
                QMessageBox.No):
            return
        sql = "DELETE FROM 用户管理 WHERE 用户名 = '%s'" % (id)
        cur = self.db.cursor()
        cur.execute(sql)
        self.db.commit()
        self.begin()
...
class addmanager(QDialog):

    def __init__(self, parent=None):
        super(addmanager, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.label_3 = QLabel("公司权限:")
        self.label_4 = QLabel("批发中心权限:")
        self.label_5 = QLabel("基地权限:")
        self.label_6 = QLabel("代购点权限:")
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""):
            ...
            cur_0 = db_0.cursor()
            sql = "SELECT * FROM 用户管理 WHERE 用户名='%s'" % (text_1)
            cur_0.execute(sql)
...
class altermanager(QDialog):

    def __init__(self,parent=None):
        super(altermanager, self).__init__(parent)
        global main_alter_record
    ...
    def setUpUI(self):
        ...
        self.db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
        self.cur = self.db.cursor()
        sql = "SELECT * FROM 用户管理 WHERE 用户名='%s'" % (self.id)
        self.cur.execute(sql)
        self.queryResults = self.cur.fetchall()
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""):
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "UPDATE 用户管理 SET 密码='%s',公司权限='%s',批发中心权限='%s',基地权限='%s',代购点权限='%s' WHERE 用户名='%s'" % (text_2,text_3,text_4,text_5,text_6,text_1)
            cur.execute(sql)
            db.commit()
            print(QMessageBox.information(self, "提示", "添加成功!", QMessageBox.Yes, QMessageBox.Yes))
            self.close()
