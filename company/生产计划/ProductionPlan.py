...
password_entry = ""
main_alter_record = ''
class ProductionPlan(QWidget):
    def __init__(self, parent=None):
        super(ProductionPlan, self).__init__()
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.layout.setContentsMargins(12, 12, 12, 55)
        ...
        searchCondision = ['按农户姓名查询', '按品种查询', '按播种期查询']
    ...
    def searchButtonClicked(self):
        ...
        if (conditionChoice == "按农户姓名查询"):
            conditionChoice = '农户姓名'
        elif (conditionChoice == "按品种查询"):
            conditionChoice = '品种'
        else:
            conditionChoice = '播种期'
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 生产计划信息表 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
    ...
    def addButtonClicked(self):
        addDialog = addProductionPlan(self)
        addDialog.show()
        addDialog.exec_()
        self.begin()
    def alterButtonClicked(self):
        ...
        alterDialog = alterProductionPlan(self)
        alterDialog.show()
        alterDialog.exec_()
        self.begin()
    def deleteButtonClicked(self):
        ...
        if (QMessageBox.warning(self, "警告", "是否删除此行", QMessageBox.Yes, QMessageBox.No) ==
                QMessageBox.No):
            return
        sql = "DELETE FROM 生产计划信息表 WHERE 序号 = '%s'" % (id)
        cur = self.db.cursor()
        cur.execute(sql)
        self.db.commit()
        self.begin()
...
class addProductionPlan(QDialog):

    def __init__(self, parent=None):
        super(addProductionPlan, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.label_1= QLabel("序号:")
        self.label_2 = QLabel("农户姓名:")
        self.label_3 = QLabel("品种:")
        self.label_4 = QLabel("面积:")
        self.label_5 = QLabel("播种期:")
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""):
            ...
            cur_0 = db_0.cursor()
            sql = "SELECT * FROM 生产计划信息表 WHERE 序号='%s'" % (text_1)
            cur_0.execute(sql)
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 生产计划信息表 VALUES ('%s','%s','%s','%s','%s','%s')" % (text_1, text_2, text_3,text_4,text_5,text_6)
            cur.execute(sql)
            db.commit()
...
class alterProductionPlan(QDialog):

    def __init__(self,parent=None):
        super(alterProductionPlan, self).__init__(parent)
        global main_alter_record
    ...
    def setUpUI(self):
        ...
        self.db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
        self.cur = self.db.cursor()
        sql = "SELECT * FROM 生产计划信息表 WHERE 序号='%s'" % (self.id)
        self.cur.execute(sql)
        self.queryResults = self.cur.fetchall()
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""):
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "UPDATE 生产计划信息表 SET 农户姓名='%s',品种='%s',面积='%s',播种期='%s',备注='%s' WHERE 序号='%s'" % (text_2,text_3,text_4,text_5,text_6,text_1)
            cur.execute(sql)
            db.commit()
            print(QMessageBox.information(self, "提示", "添加成功!", QMessageBox.Yes, QMessageBox.Yes))
            self.close()
