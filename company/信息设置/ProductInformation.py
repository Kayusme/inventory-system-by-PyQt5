...
'''总仿'''
main_alter_record = ''
class ProductInformation(QWidget):
...
class addProductInformation(QDialog):

    def __init__(self, parent=None):
        super(addProductInformation, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.label_3 = QLabel("产品等级:")
        self.label_4 = QLabel("产地:")
        self.label_5 = QLabel("进价:")
        self.label_6 = QLabel("包装物:")
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""):
            ...
            cur_0 = db_0.cursor()
            sql = "SELECT * FROM 产品信息表 WHERE 产品编号='%s'" % (text_1)
            cur_0.execute(sql)
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 产品信息表 VALUES ('%s','%s','%s','%s','%s','%s')" % (text_1, text_2, text_3,text_4,text_5,text_6)
            cur.execute(sql)
            db.commit()
...
class alterProductInformation(QDialog):

    def __init__(self,parent=None):
        super(alterProductInformation, self).__init__(parent)
        global main_alter_record
    ...
    def setUpUI(self):
        ...
        self.db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
        self.cur = self.db.cursor()
        sql = "SELECT * FROM 产品信息表 WHERE 产品编号='%s'" % (self.id)
        self.cur.execute(sql)
        self.queryResults = self.cur.fetchall()
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""):
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "UPDATE 产品信息表 SET 产品名称='%s',产品等级='%s',产地='%s',进价='%s',包装物='%s' WHERE 产品编号='%s'" % (text_2,text_3,text_4,text_5,text_6,text_1)
            cur.execute(sql)
            db.commit()
            print(QMessageBox.information(self, "提示", "添加成功!", QMessageBox.Yes, QMessageBox.Yes))
            self.close()
