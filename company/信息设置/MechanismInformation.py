...
'''总仿'''
main_alter_record = ''
class MechanismInformation(QWidget):
    def __init__(self, parent=None):
        super(MechanismInformation, self).__init__()
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        searchCondision = ['按机构编号查询', '按机构名称查询', '按机构地址查询']
    ...
    def searchButtonClicked(self):
        ...
        if (conditionChoice == "按机构编号查询"):
            conditionChoice = '机构编号'
        elif (conditionChoice == "按机构名称查询"):
            conditionChoice = '机构名称'
        else:
            conditionChoice = '机构地址'
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 公司机构设置表 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
    ...
    def addButtonClicked(self):
        addDialog = addMechanismInformation(self)
        addDialog.show()
        addDialog.exec_()
        self.begin()
    def alterButtonClicked(self):
        ...
        alterDialog = alterMechanismInformation(self)
        alterDialog.show()
        alterDialog.exec_()
        self.begin()
    def deleteButtonClicked(self):
        ...
        if (QMessageBox.warning(self, "警告", "是否删除此行", QMessageBox.Yes, QMessageBox.No) ==
                QMessageBox.No):
            return
        sql = "DELETE FROM 公司机构设置表 WHERE 机构编号 = '%s'" % (id)
        cur = self.db.cursor()
        cur.execute(sql)
        self.db.commit()
        self.begin()
...
class addMechanismInformation(QDialog):

    def __init__(self, parent=None):
        super(addMechanismInformation, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.label_1= QLabel("机构编号:")
        self.label_2 = QLabel("机构类型:")
        self.label_3 = QLabel("机构名称:")
        self.label_4 = QLabel("机构地址:")
        self.label_5 = QLabel("建设日期:")
        self.label_6 = QLabel("备注:")
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""):
            ...
            cur_0 = db_0.cursor()
            sql = "SELECT * FROM 公司机构设置表 WHERE 机构编号='%s'" % (text_1)
            cur_0.execute(sql)
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 公司机构设置表 VALUES ('%s','%s','%s','%s','%s','%s')" % (text_1, text_2, text_3,text_4,text_5,text_6)
            cur.execute(sql)
            db.commit()
...
class alterMechanismInformation(QDialog):

    def __init__(self,parent=None):
        super(alterMechanismInformation, self).__init__(parent)
        global main_alter_record
    ...
    def setUpUI(self):
        ...
        self.db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
        self.cur = self.db.cursor()
        sql = "SELECT * FROM 公司机构设置表 WHERE 机构编号='%s'" % (self.id)
        self.cur.execute(sql)
        self.queryResults = self.cur.fetchall()
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""):
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "UPDATE 公司机构表 SET 机构类型='%s',机构名称='%s',机构地址='%s',建设日期='%s',备注='%s' WHERE 机构编号='%s'" % (text_2,text_3,text_4,text_5,text_6,text_1)
            cur.execute(sql)
            db.commit()
            print(QMessageBox.information(self, "提示", "添加成功!", QMessageBox.Yes, QMessageBox.Yes))
            self.close()
...
if __name__ == "__main__":
    ...
    mainMindow = MechanismInformation()
    mainMindow.show()
    sys.exit(app.exec_())