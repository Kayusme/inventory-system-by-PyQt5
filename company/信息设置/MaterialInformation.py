...
main_alter_record = ''
class MaterialInformation(QWidget):
    def __init__(self, parent=None):
        super(MaterialInformation, self).__init__()
        self.setUpUI()
    ...
    def addButtonClicked(self):
        addDialog = addMaterialInformation(self)
        addDialog.show()
        addDialog.exec_()
        self.begin()
    ...
    def alterButtonClicked(self):
        ...
        alterDialog = alterMaterialInformation(self)
        alterDialog.show()
        alterDialog.exec_()
        self.begin()
    ...
    def deleteButtonClicked(self):
        ...
        if (QMessageBox.warning(self, "警告", "是否删除此行", QMessageBox.Yes, QMessageBox.No) ==
                QMessageBox.No):
            return
        sql = "DELETE FROM 物资信息表 WHERE 物资编号 = '%s'" % (id)
        cur = self.db.cursor()
        cur.execute(sql)
        self.db.commit()
        self.begin()
    ...
    def begin(self):
        ...
        for i in range(len(self.queryResults)):
            for j in range(len(self.col_lst)):
                ...
                self.tableWidget.setItem(i, j, data1)
        return
class addMaterialInformation(QDialog):

    def __init__(self, parent=None):
        super(addMaterialInformation, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.label_3 = QLabel("单位:")
        self.label_4 = QLabel("进价:")
        self.label_5 = QLabel("出价:")
...
class alterMaterialInformation(QDialog):

    def __init__(self,parent=None):
        super(alterMaterialInformation, self).__init__(parent)
        global main_alter_record
    ...
    def setUpUI(self):
        ...
        self.db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
        self.cur = self.db.cursor()
        sql = "SELECT * FROM 物资信息表 WHERE 物资编号='%s'" % (self.id)
        self.cur.execute(sql)
        self.queryResults = self.cur.fetchall()
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" ):
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "UPDATE 物资信息表 SET 物资名称='%s',单位='%s',进价='%s',出价='%s',备注='%s' WHERE 物资编号='%s'" % (text_2,text_3,text_4,text_5,text_6,text_1)
            cur.execute(sql)
            db.commit()
            print(QMessageBox.information(self, "提示", "添加成功!", QMessageBox.Yes, QMessageBox.Yes))
            self.close()
...
if __name__ == "__main__":
    ...
    mainMindow = MaterialInformation()
    mainMindow.show()
    sys.exit(app.exec_())