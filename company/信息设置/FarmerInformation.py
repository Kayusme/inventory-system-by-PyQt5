...
main_alter_record = ''
class FarmerInformation(QWidget):
    def __init__(self, parent=None):
        super(FarmerInformation, self).__init__()
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        searchCondision = ['按农户编号查询', '按农户名称查询', '按农户地址查询']
    ...
    def searchButtonClicked(self):
        ...
        if (conditionChoice == "按农户编号查询"):
            conditionChoice = '农户编号'
        elif (conditionChoice == "按农户名称查询"):
            conditionChoice = '农户名称'
        else:
            conditionChoice = '农户住址'
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 农户信息表 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
    ...
    def addButtonClicked(self):
        addDialog = addFarmerInformation(self)
        addDialog.show()
        addDialog.exec_()
        self.begin()
    def alterButtonClicked(self):
        ...
        alterDialog = alterFarmerInformation(self)
        alterDialog.show()
        alterDialog.exec_()
        self.begin()
    def deleteButtonClicked(self):
        ...
        if (QMessageBox.warning(self, "警告", "是否删除此行", QMessageBox.Yes, QMessageBox.No) ==
                QMessageBox.No):
            return
        sql = "DELETE FROM 农户信息表 WHERE 农户编号 = '%s'" % (id)
        cur = self.db.cursor()
        cur.execute(sql)
        self.db.commit()
        self.begin()
...
class addFarmerInformation(QDialog):

    def __init__(self, parent=None):
        super(addFarmerInformation, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.label_2 = QLabel("农户名称:")
        self.label_3 = QLabel("农户住址:")
        self.label_4 = QLabel("联系电话:")
        self.label_5 = QLabel("播种面积:")
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" ):
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 农户信息表 VALUES ('%s','%s','%s','%s','%s')" % (text_1, text_2, text_3,text_4,text_5)
            cur.execute(sql)
            db.commit()
...
class alterFarmerInformation(QDialog):

    def __init__(self,parent=None):
        super(alterFarmerInformation, self).__init__(parent)
        global main_alter_record
    ...
    def setUpUI(self):
        ...
        self.db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
        self.cur = self.db.cursor()
        sql = "SELECT * FROM 农户信息表 WHERE 农户编号='%s'" % (self.id)
        self.cur.execute(sql)
        self.queryResults = self.cur.fetchall()
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" ):
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "UPDATE 农户信息表 SET 农户名称='%s',产品等级='%s',农户住址='%s',播种面积/平方米='%s' WHERE 农户编号='%s'" % (text_2,text_3,text_4,text_5,text_1)
            cur.execute(sql)
            db.commit()
            print(QMessageBox.information(self, "提示", "添加成功!", QMessageBox.Yes, QMessageBox.Yes))
            self.close()
