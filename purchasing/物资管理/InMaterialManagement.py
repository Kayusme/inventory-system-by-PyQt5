...
class InMaterialManagement(QWidget):
    def __init__(self, parent=None):
        super(InMaterialManagement, self).__init__()
        self.setUpUI()
    def setUpUI(self):
        # 查询数据
        ...
        self.addButton.setIcon(QIcon('.//image//物资入库.png'))
        ...
        searchCondision = ['按入库单号查询','按物资编号查询','按物资名称查询']
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 物资入库信息表 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
    ...
    def addButtonClicked(self):
        addDialog = addInMaterialManagement(self)
        addDialog.show()
        addDialog.exec_()
        self.begin()
        return
...
class addInMaterialManagement(QDialog):

    def __init__(self, parent=None):
        super(addInMaterialManagement, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.label_danwei = QLabel("单位:")
        self.label_5 = QLabel("机构编号:")
        ...
        self.lineedit_danwei = QLineEdit()
        self.lineedit_danwei.setEnabled(False)
        ...
        self.lineedit_danwei.setMaxLength(10)
        ...
        self.layout.addRow(self.label_danwei, self.lineedit_danwei)
        ...
        self.lineedit_danwei.setFont(font)
    ...
    def handleTextChanged(self):
        ...
        cur = self.db.cursor()
        sql = "SELECT * FROM 物资信息表 WHERE 物资编号='%s'" % (text_1)
        cur.execute(sql)
        queryResults = cur.fetchall()
        if not queryResults:
            self.lineedit_2.setText('')
            self.lineedit_3.setText('')
            self.lineedit_danwei.setText('')
            return
        self.lineedit_2.setText(queryResults[0][1])
        self.lineedit_3.setText(queryResults[0][3])
        self.lineedit_danwei.setText(queryResults[0][2])
        return
    ####
    def buttonCicked(self):
        ...
        text_danwei = self.lineedit_danwei.text()
        text_5 = self.lineedit_5.text()

        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_danwei == ""):
            ...
            cur_0 = db_0.cursor()
            sql = "SELECT * FROM 公司机构设置表 WHERE 机构编号='%s'" % (text_5)
            cur_0.execute(sql)
            queryResults_0 = cur_0.fetchall()
            if not queryResults_0:
                print(QMessageBox.warning(self, "警告", "机构编号为[ " + text_5 + " ]不存在！", QMessageBox.Yes,
                                          QMessageBox.Yes))
                return
            # 打开数据库连接
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 物资入库信息表 VALUES ('%s','%s','%s','%s','%s','%s')" % (str(id+1),text_1, text_2, text_3,text_4,text_5)
            cur.execute(sql)
            db.commit()
            #库存
            ...
            for Result in queryResults_c:
                if Result[0]==text_1:
                    sql ="UPDATE 物资库存信息表 SET 数量='%s' WHERE 物资编号='%s'" % (str(int(text_4)+int(Result[2])),text_1)
                    cur_c.execute(sql)
                    db_c.commit()
            ...
            sql = "INSERT INTO 物资库存信息表 VALUES ('%s','%s','%s','%s','%s')" % (text_1, text_2, text_4, text_danwei,"无")
            cur_c.execute(sql)
            db_c.commit()
            print(QMessageBox.information(self, "提示", "添加成功!", QMessageBox.Yes, QMessageBox.Yes))
            self.close()
...
if __name__ == "__main__":
    ...
    mainMindow = InMaterialManagement()
    mainMindow.show()
    sys.exit(app.exec_())