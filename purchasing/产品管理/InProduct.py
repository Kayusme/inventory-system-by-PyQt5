...
class InProduct(QWidget):
    ...
    def setUpUI(self):
        ...
        searchCondision = ['按入库单号查询','按产品编号查询','按入库时间查询','按农户编号查询']
...
class addInProduct(QDialog):
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_8== "" or text_9 == ""or text_10 == ""):
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 产品收购表 VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (str(id+1),text_1, text_2, text_3
            ...
            for Result in queryResults_c:
                if Result[0]==text_1:
                    sql ="UPDATE 代购点产品库存 SET 数量='%s' WHERE 产品编号='%s'" % (str(int(text_4)+int(Result[2])),text_1)
                    cur_c.execute(sql)
                    db_c.commit()
            ...
            sql = "INSERT INTO 代购点产品库存 VALUES ('%s','%s','%s','%s')" % (text_1, text_2, text_4,text_11)
            cur_c.execute(sql)
            db_c.commit()
            print(QMessageBox.information(self, "提示", "添加成功!", QMessageBox.Yes, QMessageBox.Yes))
            self.close()
