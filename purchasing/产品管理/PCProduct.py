...
main_alter_record = ''
class PCProduct(QWidget):
    def __init__(self, parent=None):
        super(PCProduct, self).__init__()
        self.setUpUI()
    def setUpUI(self):
        # 查询数据
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 代购点产品盘存 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
...
class addPCProduct(QDialog):

    def __init__(self, parent=None):
        super(addPCProduct, self).__init__(parent)
        self.setUpUI()
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" ):
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 代购点产品盘存 VALUES ('%s','%s','%s','%s')" % (text_1, text_2, text_3,text_4)
            cur.execute(sql)
            db.commit()
