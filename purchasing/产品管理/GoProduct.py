...
class GoProduct(QWidget):
    def __init__(self, parent=None):
        super(GoProduct, self).__init__()
        self.setUpUI()
    def setUpUI(self):
        # 查询数据
        ...
        searchCondision = ['按出库单号查询','按机构编号查询','按产品编号查询']
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 代购点产品出库 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
...
class addGoProduct(QDialog):

    def __init__(self, parent=None):
        super(addGoProduct, self).__init__(parent)
        self.setUpUI()
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" ):
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 代购点产品出库 VALUES ('%s','%s','%s','%s','%s')" % (str(id+1),text_1, text_2, text_3,text_4)
            cur.execute(sql)
            db.commit()
            #库存
            db_c = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            ...
            for Result in queryResults_c:
                if Result[0]==text_1:
                    if int(text_3)>int(Result[2]):
                        ...
                        sql = "UPDATE 代购点产品库存 SET 数量='%s' WHERE 产品编号='%s'" % (str(int(Result[2])-int(text_3)), text_1)
                        cur_c.execute(sql)
                        db_c.commit()
