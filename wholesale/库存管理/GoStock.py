...
password_entry = ""
main_alter_record = ''
class GoStock(QWidget):
    def __init__(self, parent=None):
        super(GoStock, self).__init__()
        self.setUpUI()
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 销售统计表 WHERE %s LIKE '%s' ORDER BY %s " % (
    ...
    def deleteButtonClicked(self):
        ...
        if (QMessageBox.warning(self, "警告", "是否删除此行", QMessageBox.Yes, QMessageBox.No) ==
                QMessageBox.No):
            return
        sql = "DELETE FROM 销售统计表 WHERE 销售单号 = '%s'" % (id)
        cur = self.db.cursor()
        cur.execute(sql)
        self.db.commit()
        self.begin()
    ...
    def begin(self):
        ...
        for i in range(len(self.queryResults)):
            for j in range(len(self.col_lst)):
                ...
                self.tableWidget.setItem(i, j, data1)
        return
class addGoStock(QDialog):

    def __init__(self, parent=None):
        super(addGoStock, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        self.label_1= QLabel("销售单号:")
        self.label_2 = QLabel("产品编号:")
        self.label_3 = QLabel("产品名称:")
        self.label_4 = QLabel("售出数量:")
        self.label_5 = QLabel("售出价格:")
        self.label_6 = QLabel("销售日期:")
        self.label_7 = QLabel("客户编号:")
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == "" or text_7 == ""):
            ...
            cur_0 = db_0.cursor()
            sql = "SELECT * FROM 销售统计表 WHERE 销售单号='%s'" % (text_1)
            cur_0.execute(sql)
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 销售统计表 VALUES ('%s','%s','%s','%s','%s','%s','%s')" % (text_1, text_2, text_3,text_4,text_5,text_6,text_7)
            cur.execute(sql)
            db.commit()
