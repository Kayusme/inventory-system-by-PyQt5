...
password_entry = ""
main_alter_record = ''
class ProductInstock(QWidget):
    def __init__(self, parent=None):
        super(ProductInstock, self).__init__()
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        # self.alterButton = QPushButton("修 改")
        ...
        searchCondision = ['入库单号','产品编号','产品名称']
    ...
    def searchButtonClicked(self):
        ...
        if (conditionChoice == "入库单号"):
            conditionChoice = '单号'
        elif (conditionChoice == "产品编号"):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 基地产品出库表 WHERE %s LIKE '%s' ORDER BY %s " % (
    ...
    def addButtonClicked(self):
        addDialog = addProductInstock(self)
        addDialog.show()
        addDialog.exec_()
        self.begin()
    ...
    #     alterDialog = alterProuductInstock(self)
    #     alterDialog.show()
    #     alterDialog.exec_()
    #     self.begin()
    ...
    def deleteButtonClicked(self):
        ...
        if (QMessageBox.warning(self, "警告", "是否删除此行", QMessageBox.Yes, QMessageBox.No) ==
                QMessageBox.No):
            return
        sql = "DELETE FROM 基地产品出库表 WHERE 单号='%s'" % (id)
        cur = self.db.cursor()
        cur.execute(sql)
        self.db.commit()
        self.begin()
    ...
    def begin(self):
        ...
        for i in range(len(self.queryResults)):
            for j in range(len(self.col_lst)):
                ...
                self.tableWidget.setItem(i, j, data1)
        return
class addProductInstock(QDialog):

    def __init__(self, parent=None):
        super(addProductInstock, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        # Label控件
        self.titlelabel = QLabel("单号填写")
        self.label_1= QLabel("入库单号:")
        self.label_2 = QLabel("产品编号:")
        self.label_3 = QLabel("产品名称:")
        self.label_4 = QLabel("进价:")
        self.label_5 = QLabel("数量:")
        self.label_6 = QLabel("金额:")
        self.label_7 = QLabel("经手人:")
        self.label_8 = QLabel("入库时间:")
        self.label_9 = QLabel("片名:")
        self.label_10 = QLabel("收获中心:")
        self.label_11 = QLabel("机构编号:")
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""or text_7 == ""or text_8 == ""  or text_9 == "" or text_10 == "" or text_11 == ""):
            ...
            cur_0 = db_0.cursor()
            sql = "SELECT * FROM 基地产品出库表 WHERE 单号='%s'" % (text_1)
            cur_0.execute(sql)
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 基地产品出库表 VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (text_1, text_2, text_3,text_4,text_5,text_6,text_7,text_8,text_9,text_10,text_11)
            cur.execute(sql)
            db.commit()
