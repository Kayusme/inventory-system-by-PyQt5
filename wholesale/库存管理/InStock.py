...
password_entry = ""
main_alter_record = ''
class InStock(QWidget):
    def __init__(self, parent=None):
        super(InStock, self).__init__()
        self.setUpUI()
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 产品库存表 WHERE %s LIKE '%s' ORDER BY %s " % (
    ...
    def addButtonClicked(self):
        addDialog = addProductInventory(self)
        addDialog.show()
        addDialog.exec_()
        self.begin()
    ...
    #             QMessageBox.No):
    #         return
    #     sql = "DELETE FROM 产品库存表 WHERE 产品编号 = '%s'" % (id)
    #     cur = self.db.cursor()
    #     cur.execute(sql)
    #     self.db.commit()
    #     self.begin()
    ...
    def begin(self):
        ...
        for i in range(len(self.queryResults)):
            for j in range(len(self.col_lst)):
                ...
                self.tableWidget.setItem(i, j, data1)
        return
class addProductInventory(QDialog):

    def __init__(self, parent=None):
        super(addProductInventory, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        # Label控件
        self.titlelabel = QLabel(" 盘存单填写：")
        self.label_1= QLabel("产品编号:")
        self.label_2 = QLabel("名称:")
        self.label_3 = QLabel("等级:")
        self.label_4 = QLabel("数量:")
        self.label_5 = QLabel("条码:")
        self.label_6 = QLabel("机构编码:")
        self.label_7 = QLabel("备注:")
    ...
    def buttonCicked(self):
        ...
        if (text_1 == "" or text_2 == "" or text_3 == "" or text_4 == ""  or text_5 == "" or text_6 == ""or text_7 == ""):
            ...
            cur_0 = db_0.cursor()
            sql = "SELECT * FROM 产品库存表 WHERE 产品编号='%s'" % (text_1)
            cur_0.execute(sql)
            ...
            db = pymysql.connect(host=host_entry, user="root", password=password_entry, db="sleep")
            cur = db.cursor()
            sql = "INSERT INTO 产品库存表 VALUES ('%s','%s','%s','%s','%s','%s','%s')" % (text_1, text_2, text_3,text_4,text_5,text_6,text_7)
            cur.execute(sql)
            db.commit()
