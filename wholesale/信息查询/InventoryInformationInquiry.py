...
password_entry = ""
main_alter_record = ''
class InventoryInformationInquiry(QWidget):
    def __init__(self, parent=None):
        super(InventoryInformationInquiry, self).__init__()
        self.setUpUI()
    def setUpUI(self):
        # 查询数据
    ...
    def searchButtonClicked(self):
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = ("SELECT * FROM 产品库存表 WHERE %s LIKE '%s' ORDER BY %s " % (conditionChoice, s, conditionChoice))
...
if __name__ == "__main__":
    ...
    mainMindow = InventoryInformationInquiry()
    mainMindow.show()
    sys.exit(app.exec_())